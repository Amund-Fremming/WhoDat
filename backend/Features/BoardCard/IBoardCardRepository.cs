...
public interface IBoardCardRepository
{
    ...
    Task CreateBoardCards(IEnumerable<BoardCardEntity> boardCards);
    ...
    Task UpdateBoardCardsActivity(IDictionary<int, bool> updateMap, IEnumerable<BoardCardEntity> boardCards);

    /// <summary>
    /// Retrieves all the BoardCards from a given board.
    /// </summary>
    /// <param name="boardId">Id of the board to get cards from.</param>
    Task<IList<BoardCardEntity>> GetBoardCardsFromBoard(int boardId);
}